...
namespace Graphs_Viewer
{
    ...
    class UndirectedGraph
    {
        ...
        public UndirectedGraph(Form _ControlerForm)
        {
        ...
        private Edge GetEdge(Vertex V1, Vertex V2)
        {
            foreach (Edge E in Edges)
            {
                ...
                if (E.Vertex1.Value == V2.Value && E.Vertex2.Value == V1.Value) return E;
            }
            return null;
        }
        /// <summary>
        ...
        private void ReadAdjacencyMatrix(string DataFilePath)
        {
            ...
            for (int i = 0; i < AdjacencyMatrixSize; i++)
            {
                ...
                bool isEmptyVertex = true;
                for (int j = 0; j < Split.Length; j++)
                {
                    AdjacencyMatrix[i, j] = Convert.ToInt32(Split[j]);
                    if (AdjacencyMatrix[i, j] > 0) isEmptyVertex = false;
                }
                if (!isEmptyVertex)
                {
        ...
        private void DepthFirstSearch_Recursion(int[,] AdjacencyMatrix, int StartVertex, ref bool[] Free)
        {
            ...
            for (int i = 0; i <= AdjacencyMatrix.GetUpperBound(1); i++)
                if (AdjacencyMatrix[StartVertex, i] != 0 && Free[i] && StartVertex != i)
                    DepthFirstSearch_Recursion(AdjacencyMatrix, i, ref Free);
        }
        /// <summary>
        ...
        /// <param name="AdjacencyMatrix">Ma trận kề</param>
        /// <param name="StartVertex">Đỉnh bắt đầu</param>
        private void DepthFirstSearch_Stack(int[,] AdjacencyMatrix, int StartVertex, ref bool[] Free)
        {
            ...
            while (VerticesStack.Count > 0)
            {
                ...
                ReDraw();
                for (int i = AdjacencyMatrix.GetUpperBound(0); i >= 0; i--)
                    if (Free[i] && AdjacencyMatrix[VertexIndex, i] != 0 && VertexIndex != i)
                    {
        ...
        /// <param name="Free">Mảng đánh dấu đã duyệt qua</param>
        private void BreadthFirstSearch_Recursion(int[,] AdjacencyMatrix, List<int> StartVertices, ref bool[] Free)
        {
            ...
            for (int i = 0; i < StartVertices.Count; i++)
            {
                ...
                for (int j = 0; j <= AdjacencyMatrix.GetUpperBound(0); j++)
                    if (AdjacencyMatrix[StartVertices[i], j] != 0 && Free[j] && !Sub.Contains(j))
                        Sub.Add(j);
            }
            if (Sub.Count > 0)
                BreadthFirstSearch_Recursion(AdjacencyMatrix, Sub, ref Free);
            Sub.Clear();
            Sub = null;
        }
        /// <summary>
        ...
        /// <param name="AdjacencyMatrix">Ma trận kề</param>
        /// <param name="StartVertex">Đỉnh bắt đầu</param>
        private void BreadthFirstSearch_Queue(int[,] AdjacencyMatrix, int StartVertex, ref bool[] Free)
        {            
            ...
            while (QU.Count != 0)
            {
                ...
                ReDraw();
                for (int v = 0; v <= AdjacencyMatrix.GetUpperBound(0); v++)
                    if (Free[v] && AdjacencyMatrix[aVertex, v] != 0 && aVertex != v)
                    {
                        QU.Enqueue(v);
                        Free[v] = false;
                    }
        ...
        /// <summary>
        /// Tìm đường đi ngắn nhất giữa 2 đỉnh dùng giải thuật Dijsktra
        /// </summary>
        /// <param name="AdjacencyMatrix">Ma trận kề</param>
        /// <param name="SourceVertexValue">Giá trị của đỉnh bắt đầu</param>
        /// <param name="TargetVertexValue">Giá trị của đỉnh kết thúc</param>
        private void DijsktraAlgorithm(int[,] AdjacencyMatrix, int SourceVertexValue, int TargetVertexValue)
        {
            int AdjacencyMatrixSize = AdjacencyMatrix.GetUpperBound(0) + 1;
            bool[] isFree = new bool[AdjacencyMatrixSize];  //Mảng trạng thái
            int[] Trace = new int[AdjacencyMatrixSize];     // mảng truy vết đường đi
            int[] Distance = new int[AdjacencyMatrixSize];  // khoảng cách từ đỉnh xuất phát tới các dỉnh còn lại
            for (int i = 0; i < AdjacencyMatrixSize; i++)
            {
                isFree[i] = true;
                Distance[i] = 9999;
            ...
            while (IsNotFound)
            {
                ...
                ReDraw();
                //// Đỉnh mới tìm được, nếu = đỉnh kết thúc thì dừng vòng lặp
                if (MinDistanceVertex == -1 || MinDistanceVertex == TargetVertexValue)
                {
                    ...
                    for (Neighbor = 0; Neighbor < AdjacencyMatrixSize; Neighbor++)
                        if (isFree[Neighbor] && AdjacencyMatrix[MinDistanceVertex, Neighbor] != 0)
                        {
                            bool KT = Distance[Neighbor] > Distance[MinDistanceVertex] + AdjacencyMatrix[MinDistanceVertex, Neighbor];
                            ...
                            {
                                VeTemp.Infor = Distance[Neighbor].ToString() + " > " + Distance[MinDistanceVertex].ToString() + " + " + AdjacencyMatrix[MinDistanceVertex, Neighbor].ToString() + " ?";
                                ReDraw();
                            ...
                            if (KT)
                            {
                                Distance[Neighbor] = Distance[MinDistanceVertex] + AdjacencyMatrix[MinDistanceVertex, Neighbor];
        ...
        /// Chạy Dijsktra
        /// </summary>
        private void RunDijsktra()
        {
            try
            {
                ResetVetiesValue();
                int Vertex1Value = InputVertex("Nhập giá trị của đỉnh bắt đầu:", "Dijsktra [1/2]", false);
                Vertex Vertex1 = GetVertex(Vertex1Value);
                int Vertex2Value = InputVertex("Nhập giá trị của đỉnh kết thúc:", "Dijsktra [2/2]", false);
                Vertex Vertex2 = GetVertex(Vertex2Value);
                MessageBox.Show("Bắt đầu chạy Dijsktra", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                DijsktraAlgorithm(GetAdjacencyMatrix(), Vertex1Value - 1, Vertex2Value - 1);
            }
            catch (System.Exception ex)
            {
                MessageBox.Show(ex.Message + " Sẽ không chạy Dijsktra", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        /// <summary>
        ...
        /// Mở ma trận kề
        /// </summary>
        public void OpenAdjacencyMatrix()
        {
            ...
            if (Dialog.ShowDialog() == DialogResult.OK)
            {
                ReadAdjacencyMatrix(Dialog.FileName);
        ...
        public void SaveAdjacencyMatrixWithRectangle()
        {
            System.Windows.Forms.SaveFileDialog Dialog = new System.Windows.Forms.SaveFileDialog();
            //Dialog.InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Personal);
            Dialog.Filter = "File ma trận kề có vị trí các đỉnh (*.AMR)|*.ARM|All Files (*.*)|*.*";
            Dialog.FilterIndex = 1;
            if (Dialog.ShowDialog() == DialogResult.OK)
            {
        ...
        /// <summary>
        /// Mở ma trận kề có toạ độ của các đỉnh
        /// </summary>
        public void OpenAdjacencyMatrixWithRectangle()
        {
            System.Windows.Forms.OpenFileDialog Dialog = new System.Windows.Forms.OpenFileDialog();
            //Dialog.InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Personal);
            Dialog.Filter = "File ma trận kề có vị trí các đỉnh (*.AMR)|*.ARM|All Files (*.*)|*.*";
            ...
            if (Dialog.ShowDialog() == DialogResult.OK)
            {
                ReadAdjacencyMatrixWithRectangle(Dialog.FileName);
                MessageBox.Show("Đã đọc đồ thị từ file xong", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ControlerForm.Invalidate();
            }
        }
        ...
        {
            RunInThread(new ThreadStart(RunDijsktra));
        }
        /// <summary>
        ...
        public void AddAVertexHereToolStripMenuItem()
        {
            ...
            {
                int NewVertexValue = InputVertex("Nhập giá trị của đỉnh:", "Thêm đỉnh mới [1/3]", true);
                Vertex _Vtemp = new Vertex(MenuLocation, NewVertexValue);
                int NumberOfVertexConnected = InputNumber("Nhập số lượng đỉnh có kết nối với đỉnh này:", "Thêm đỉnh mới [2/3]", false);
                for (int i = 0; i < NumberOfVertexConnected; i++)
                {
                    int ConnectedVertexValue = InputVertex("Cạnh thứ " + (i + 1).ToString() + " nối từ đỉnh " + NewVertexValue + " tới đỉnh: ", "Thêm đỉnh mới [2." + (i + 1).ToString() + "a/2]", false);
        ...
        {
            ViewMatrix("Ma trận kề", ViewMatrixStyle.AdjacencyMatrix);
        }
        /// <summary>
        ...
        {
            ViewMatrix("Ma trận liên thuộc", ViewMatrixStyle.IncidenceMatrix);
        }
        /// <summary>
        /// Hiên danh sách kề
        /// </summary>
        ...
        {
            ViewMatrix("Danh sách kề", ViewMatrixStyle.AdjacencyList);
        }
        /// <summary>
        ...
        {
            ViewMatrix("Danh sách cạnh", ViewMatrixStyle.IncidenceList);
        }
        #endregion
